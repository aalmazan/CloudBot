...
def speedtest_url(match):
    ...
    download = (
        data.xpath('//div[@class="share-speed share-download"]/p')[0]
        .text_content()
        .strip()
    )
    upload = (
        data.xpath('//div[@class="share-speed share-upload"]/p')[0]
        .text_content()
        .strip()
    )

    ping = (
        data.xpath('//div[@class="share-data share-ping"]/p')[0].text_content().strip()
    )
    isp = (
        data.xpath('//div[@class="share-data share-isp"]/p')[0]
        .text_content()
        .strip()
        .title()
    )
