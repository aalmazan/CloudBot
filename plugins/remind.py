...
async def add_reminder(
    async_call, db, network, added_user, added_chan, message, remind_time, added_time
):
...
async def remind(text, nick, chan, db, conn, event, async_call):
    ...
    count = len(
        [x for x in reminder_cache if x[0] == conn.name and x[3] == nick.lower()]
    )
    ...
    await add_reminder(
        async_call, db, conn.name, nick, chan, message, remind_time, current_time
    )
    await load_cache(async_call, db)
