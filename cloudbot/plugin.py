...
class PluginManager:
    ...
    def can_load(self, plugin_title, noisy=True):
        ...
        if pl.get("use_whitelist", False):
            if plugin_title not in pl.get("whitelist", []):
                if noisy:
                    logger.info(
                        'Not loading plugin module "%s": ' "plugin not whitelisted",
                        plugin_title,
                    )
        ...
        if plugin_title in pl.get("blacklist", []):
            if noisy:
                logger.info(
                    'Not loading plugin module "%s": plugin blacklisted', plugin_title
                )
    ...
    async def _sieve(self, sieve, event, hook):
        ...
        try:
            ...
            logger.exception(
                "Error running sieve %s on %s:", sieve.description, hook.description
            )
            error = sys.exc_info()
    ...
    async def create_tables(self, bot):
        ...
        if self.tables:
            ...
            for table in self.tables:
                if not (
                    await bot.loop.run_in_executor(None, table.exists, bot.db_engine)
                ):
